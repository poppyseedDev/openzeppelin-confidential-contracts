...
const duration = 1234;
const cliff = 10;
const extraData = ethers.hexlify('0x1234');
const amount1 = 101;
const amount2 = 102;
...
describe('VestingWalletCliffExecutorConfidentialFactory', function () {
  ...
  it('should create vesting wallet', async function () {
    ...
    await expect(vestingWallet.extraData()).to.eventually.equal(extraData);
  });
  ...
  it('should be able to claim tokens after creating wallet', async function () {
    ...
    const vestingWalletParams = [this.recipient, startTimestamp, duration, cliff, this.executor, extraData];
